...
const PAC_URL = "https://p.thenewone.lol:8443/proxy.pac"
...
var CACHE_DIR = filepath.Join(PATH_TEMP, "antizapret")
...
func isInNet(ip net.IP, netaddr string, netmask int) bool {
	...
	if err != nil {
		log.Printf("Error parsing network %s/%d: %s\n", netaddr, netmask, err)
		return false
	}
...
func loadConfig() (*AntizapretConfig, error) {
	...
	if _config != nil && time.Since(_config.CreatedAt) <= CACHE_TTL {
		...
		configMutex.RUnlock()
		log.Println("Using cached Antizapret config (in memory)")
		return _config, nil
	}
	configMutex.RUnlock()
	...
	if _config != nil && time.Since(_config.CreatedAt) <= CACHE_TTL {
		log.Println("Using cached Antizapret config (in memory, double-checked)")
		return _config, nil
	}
	...
	if cacheFilePath != "" {
		...
		if err == nil {
			...
			log.Println("Using cached Antizapret config (from file)")
			return _config, nil
		}
		// Cache file load failed or expired, proceed to fetch
		log.Printf("Cache file '%s' load failed or expired: %v. Fetching new config.\n", cacheFilePath, err)
	} else {
		return nil, errors.New("File caching is disabled.")
	}

	// Fetch Antizapret PAC file
	log.Println("Fetching Antizapret PAC file")
	...
	log.Println("Parsing Antizapret PAC file")
	...
	*/
	...
	for _, tldEntry := range tldEntries {
		...
		if len(tldMatch) < 3 {
			log.Printf("Warning: Failed to parse TLD entry: %s\n", tldEntry)
			continue
		}
		tld := tldMatch[1]
		...
		lengthEntries := strings.SplitSeq(lengthsContent, ",")
		for lengthEntryStr := range lengthEntries {
			...
			if len(parts) < 2 {
				log.Printf("Warning: Failed to parse length entry: %s\n", lengthEntryStr)
				continue
			}
			length, err := strconv.Atoi(strings.TrimSpace(parts[0]))
			if err != nil {
				log.Printf("Warning: Failed to parse length integer '%s': %v\n", parts[0], err)
				continue
			}
			dataLength, err := strconv.Atoi(strings.TrimSpace(parts[1]))
			if err != nil {
				log.Printf("Warning: Failed to parse data length integer '%s': %v\n", parts[1], err)
				continue
			}
	...
	for _, entry := range specialEntries {
		...
		if len(parts) == 2 {
			...
			if err != nil {
				log.Printf("Warning: Failed to parse netmask integer '%s': %v\n", parts[1], err)
				continue
			}
			special = append(special, SpecialEntry{Netaddr: netaddr, Netmask: netmask})
		} else {
			log.Printf("Warning: Failed to parse special entry: %s\n", entry)
		}
	}
	...
	for _, entry := range patternEntries {
		...
		if len(parts) == 2 {
			...
			log.Printf("Warning: Failed to parse pattern entry: %s\n", entry)
		}
	}
	...
	log.Println("Initializing Antizapret config")
	...
	for _, ipStr := range dIPAddrStr {
		...
		if err != nil {
			log.Printf("Warning: Failed to parse base36 IP '%s': %v\n", ipStr, err)
		}
...
UNZLP_START:
		...
		for j := range domains[i].Lengths {
			...
			if len(leftover) < dmnl { // Need to unpack string
				...
				if len(domainsLZPBytes) == 0 || len(maskLZPBytes) == 0 {
					log.Println("Warning: Ran out of LZP data during initialization.")
					break UNZLP_START
				}
			...
			if len(leftover) < dmnl {
				log.Printf("Warning: Decompression did not yield enough data for domains['%s']['%d'] (needed %d, got %d). Storing partial data.\n", domains[i].TLD, domains[i].Lengths[j].Length, dmnl, len(leftover))
			}
	...
	if cacheFilePath != "" {
		if err := saveConfigToFile(cacheFilePath, _config); err != nil {
			log.Printf("Warning: Failed to save config to cache file '%s': %v\n", cacheFilePath, err)
			// Continue without saving, config is in memory
		} else {
			log.Println("Saved config to cache file")
		}
	}
...
func (ap *AntizapretProxy) ProxyURL(r *http.Request) (*url.URL, error) {
	if proxy := ap.Detect(r.URL.Host); proxy != "" {
		log.Printf("Using Antizapret proxy %s for %s", proxy, r.URL.Host)
